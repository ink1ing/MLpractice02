...
class AUDBlock(nn.Module):
    """
    AUD (Attentive Unified Diffusion) 基本模块
    结合了残差连接、通道注意力和空间注意力，并添加了扩散机制以捕获多尺度信息
    """
    def __init__(self, in_channels, out_channels, stride=1, expansion=4, kernel_sizes=[3, 5, 7]):
        """
        初始化AUD块
        ...
        参数:
            ...
            out_channels: 输出通道数
            stride: 第一个卷积层的步长，用于下采样，默认为1
            expansion: 扩展因子，用于中间层通道数扩展，默认为4
            kernel_sizes: 多尺度卷积核大小列表，默认为[3, 5, 7]
        """
        super(AUDBlock, self).__init__()
        ...
        for k_size in kernel_sizes:
            padding = k_size // 2  # 保持特征图大小不变
            self.diffusion_convs.append(
                nn.Sequential(
                    nn.Conv2d(bottleneck_channels, bottleneck_channels, 
                              kernel_size=k_size, stride=stride, padding=padding, bias=False),
                    nn.BatchNorm2d(bottleneck_channels),
                    nn.ReLU(inplace=True)
                )
            )
        ...
        self.fusion = nn.Sequential(
            nn.Conv2d(bottleneck_channels * len(kernel_sizes), out_channels, 
    ...
    def forward(self, x):
        ...
        返回:
            out: AUD块的输出，形状为 [batch_size, out_channels, height/stride, width/stride]
        """
        residual = x  # 保存输入作为残差连接
        ...
        for conv in self.diffusion_convs:
            diffusion_outputs.append(conv(x))
        
        # 合并多尺度特征 (通道维度上拼接)
        x = torch.cat(diffusion_outputs, dim=1)
...
class DiseaseAUDNet(nn.Module):
    """
    基于AUD (Attentive Unified Diffusion) 结构的双头模型，用于玉米南方锈病多任务分类
    使用多尺度扩散卷积、通道注意力和空间注意力增强特征提取能力
    """
    def __init__(self, in_channels=3, img_size=128):
        """
        初始化DiseaseAUDNet模型
        ...
        参数:
            ...
            img_size: 输入图像的尺寸，默认为128x128
        """
        super(DiseaseAUDNet, self).__init__()
        self.in_channels = in_channels
        ...
        # AUD块 - 构建四层网络
        self.layer1 = self._make_layer(64, 64, 2)  # 第一层：64->64通道，2个AUD块
        self.layer2 = self._make_layer(64, 128, 2, stride=2)  # 第二层：64->128通道，2个AUD块，下采样
        self.layer3 = self._make_layer(128, 256, 2, stride=2)  # 第三层：128->256通道，2个AUD块，下采样
        self.layer4 = self._make_layer(256, 512, 2, stride=2)  # 第四层：256->512通道，2个AUD块，下采样
        
        # 全局平均池化 - 降低参数量并保留空间特征
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))  # 输出1x1特征图
        
        # 特征统一层 - 将最后一层特征映射到统一空间
        self.feature_unifier = nn.Sequential(
            nn.Flatten(),
            nn.Dropout(0.5),
            nn.Linear(512, 256),
            nn.ReLU(inplace=True)
    ...
    def _make_layer(self, in_channels, out_channels, blocks, stride=1):
        ...
        参数:
            ...
            out_channels: 输出通道数
            blocks: AUD块数量
            stride: 第一个AUD块的步长，用于下采样
            
        返回:
            nn.Sequential: 包含多个AUD块的顺序容器
        """
        layers = []
        # 第一个block可能需要调整维度
        layers.append(AUDBlock(in_channels, out_channels, stride))
        # 剩余blocks保持维度不变
        for _ in range(1, blocks):
            layers.append(AUDBlock(out_channels, out_channels))
        return nn.Sequential(*layers)
    
    def _initialize_weights(self):
        """初始化模型权重，使用Kaiming初始化方法"""
    ...
    def forward(self, x):
        ...
        返回:
            tuple: (position_logits, grade_logits)
                - position_logits: 位置分类的logits，形状为 [batch_size, 3]
                - grade_logits: 等级回归值，形状为 [batch_size, 1]
        """
        # 特征提取
        x = self.conv1(x)  # 初始卷积
        x = self.bn1(x)
...
def get_model(model_type='simple', in_channels=3, img_size=128):
    ...
    参数:
        model_type (str): 模型类型，'simple', 'resnet', 'resnet+', 或 'aud'
        in_channels (int): 输入通道数，默认为3
        img_size (int): 输入图像大小，默认为128
    ...
    if model_type == 'simple':
        ...
        return DiseaseAUDNet(in_channels, img_size)
    else:
        raise ValueError(f"未知的模型类型: {model_type}. 支持的类型: 'simple', 'resnet', 'resnet+', 'aud'")
# 核心文件：修复版本的数据加载器，解决了JSON目录结构映射问题
import os
import torch
...
class CornRustDataset(Dataset):
    ...
    """
    def __init__(self, data_root, json_root=None, transform=None, img_size=128, use_extended_dataset=True):
        """
        初始化玉米南方锈病数据集
        
        参数:
            data_root (str): .tif文件的数据集目录，包含多光谱图像
            json_root (str, optional): .json标注文件目录，如果为None，则使用data_root + '_json'
        ...
        self.json_root = json_root if json_root else data_root + '_json'
        ...
        print("正在创建数据集，数据根目录:", data_root)
        
        # 获取所有样本文件路径对
        self.samples = self._get_samples()
        
        print(f"数据集创建成功，样本数量: {len(self.samples)}")
        
        # 移除初始化时的标签缓存，改为延迟加载
        self.position_labels = None
        self.grade_labels = None
    ...
    def _get_samples(self):
        ...
        if self.use_extended_dataset:
            # 直接列出数据根目录下的所有子目录
            subdirs = [d for d in os.listdir(self.data_root) if os.path.isdir(os.path.join(self.data_root, d))]
            
            # 过滤出叶片目录（9l, 9m, 9t, 14l, 14m, 14t, 19l, 19m, 19t）
            leaf_dirs = []
            for subdir in subdirs:
                if subdir in ['9l', '9m', '9t', '14l', '14m', '14t', '19l', '19m', '19t']:
                    leaf_dirs.append(os.path.join(self.data_root, subdir))
            
            if not leaf_dirs:
                print(f"警告: 未找到扩展数据集目录，仅使用当前目录: {self.data_root}")
                leaf_dirs = [self.data_root]
            
            # 处理找到的叶片目录
            for leaf_dir in leaf_dirs:
                dir_name = os.path.basename(leaf_dir)
                
                # 构建JSON子目录路径 - 修复JSON目录结构匹配
                # 例如: 9l -> 9l_json
                json_subdir = dir_name + '_json'
                json_dir = os.path.join(self.json_root, json_subdir)
            ...
            tif_files = [f for f in os.listdir(self.data_root) if f.endswith('.tif')]
            
            # 遍历tif文件，找到对应的json文件
            for tif_file in tif_files:
                tif_path = os.path.join(self.data_root, tif_file)
                # 找到对应的json文件
                json_file = tif_file.replace('.tif', '.json')
    ...
    def _cache_labels(self):
        """
        延迟缓存所有样本的标签，用于计算类别权重和统计分布
        只在需要时调用一次
        """
        if self._labels_cached:
            return
        ...
        # 遍历所有样本解析标签
        for i, (_, json_path) in enumerate(self.samples):
            if i % 100 == 0:
                print(f"  处理标签 {i}/{len(self.samples)}")
    ...
    def _parse_json_label(self, json_path):
        ...
        try:
            ...
            for component in path_components:
                # 检查目录或文件名是否包含位置标识
                if '9l' in component or '14l' in component or '19l' in component:
                    position = 'l'  # 下部
                    break
                elif '9m' in component or '14m' in component or '19m' in component:
                    position = 'm'  # 中部
                    break
                elif '9t' in component or '14t' in component or '19t' in component:
                    position = 't'  # 上部
                    break
        ...
        try:
            ...
            try:
                ...
                with rasterio.open(tif_path) as src:
                    # 避免读取全部波段，只读取我们需要的波段
                    selected_bands = [0, min(250, src.count-1), min(499, src.count-1)]  # 第一个、中间和最后一个波段
...
def get_dataloaders(data_root, json_root=None, batch_size=32, num_workers=4, img_size=128, 
    ...
    参数:
        data_root (str): TIF图像数据根目录
        json_root (str, optional): JSON标注数据根目录，默认为data_root对应的标注目录
        batch_size (int): 批次大小，默认32
        num_workers (int): 数据加载线程数，默认4
        img_size (int): 图像尺寸，默认128
        train_ratio (float): 训练集比例，默认0.8
        aug_prob (float): 数据增强概率，默认0.5
        use_extended_dataset (bool): 是否使用扩展数据集，默认True
        pin_memory (bool): 是否使用pin_memory，默认True
        prefetch_factor (int): 预取因子，默认2
        
    返回:
        tuple: (train_loader, val_loader) 训练和验证数据加载器
    """
    print("=== DataLoader 配置信息 ===")
    ...
    # 创建数据增强
    transform = DataAugmentation(aug_prob=aug_prob)

...
class FCNBackbone(nn.Module):
    def __init__(self, in_channels=3, features=[64, 128, 256, 512, 1024]):
        ...
        for feature in features:
            self.downs.append(
                nn.Sequential(
                    nn.Conv2d(in_features, feature, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, x):
        ...
        for down in self.downs[:-1]:  # 除了最后一层，都作为skip connections
            x = down(x)
        ...
        # 保存中间特征，用于AttentionGate
        self.features = skip_connections
...
class DiseaseFCN(nn.Module):
    def __init__(self, in_channels=3, position_classes=3, img_size=128):
        super(DiseaseFCN, self).__init__()
        self.img_size = img_size
        features = [64, 128, 256, 512, 1024]
        
        # 下采样路径 - 使用VGG风格的backbone
        self.backbone = FCNBackbone(in_channels, features)
        ...
        for feature in reversed(features[:-1]):
            self.ups.append(
                nn.ConvTranspose2d(
                    features[-1] if len(self.ups) == 0 else feature * 2,
                    feature,
                    kernel_size=2,
                    stride=2,
                )
            ...
            # 添加注意力门
            if len(self.ups) // 2 < len(features) - 1:
                self.attention_gates.append(
                    AttentionGate(
                        F_g=feature,                   # 上采样特征通道数
                        F_l=feature,                   # 同级skip connection特征通道数
                        F_int=feature // 2,            # 中间特征通道数
                    )
                )
        ...
        self.position_head = nn.Sequential(
            ...
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, position_classes, kernel_size=1)
        )
        ...
        self.grade_head = nn.Sequential(
            ...
            nn.Sigmoid()  # 将输出归一化到0-1范围
        )
        
        # 可视化需要的注意力图
        self.attention_maps = None
        
        # 使用torch.compile加速模型前向传播（仅在PyTorch 2.0及以上版本有效）
        if hasattr(torch, 'compile'):
            try:
                self = torch.compile(self)
                print("已使用torch.compile优化模型")
    ...
    def forward(self, x):
        ...
        # Backbone前向传播 - 获取encoder特征和skip connections
        x, skip_connections = self.backbone(x)
        
        # 反转skip connections用于上采样路径
        ...
        for idx in range(0, len(self.ups), 2):
            ...
            if idx//2 < len(self.attention_gates):
                # 保存原始skip connection以计算注意力图
                orig_skip = skip_connection
                ...
                with torch.no_grad():
                    # 计算当前层注意力图
                    # 获取注意力权重 (B, 1, H, W)
                    g1 = attention_gate.W_g(x)
                    x1 = attention_gate.W_x(orig_skip)
                    psi = attention_gate.relu(g1 + x1)
                    attention_map = attention_gate.psi(psi)
        ...
        grade_values = self.grade_head(x)
        
        # 返回位置分类logits和严重程度值
        return position_logits, grade_values.view(x.size(0), -1)

def get_model(model_type='fcn', in_channels=3, img_size=128):
    """
    获取模型实例
    
    参数:
        model_type: 模型类型，'fcn'或'resnet'
        in_channels: 输入通道数
        img_size: 图像大小
    ...
    """
    if model_type == 'fcn':
        model = DiseaseFCN(in_channels=in_channels, position_classes=3, img_size=img_size)
    else:

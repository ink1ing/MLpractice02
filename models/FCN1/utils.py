...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        参数:
            inputs: 模型输出的logits，可能的形状为:
                   [N, C] - 标准分类问题
                   [N, C, H, W] - 分割问题(FCN输出)
                   [N] 或 [N, 1] - 回归问题
            targets: 真实标签，形状取决于任务类型
            
        返回:
            loss: 计算的损失值
        """
        # 初始化loss为None，在后面会根据不同情况计算
        loss = None
        
        # 处理FCN输出的特殊情况 [N, C, H, W]
        if len(inputs.shape) == 4 and len(targets.shape) == 1:
            # FCN分割网络输出到分类任务的处理
            # 将[N, C, H, W]转换为[N, C]进行分类损失计算
            inputs = F.adaptive_avg_pool2d(inputs, 1).view(inputs.size(0), -1)
            
        # 检查输入维度，如果inputs是[N, C]而targets是[N]，则进行one-hot编码
        if len(inputs.shape) != len(targets.shape) and inputs.size(0) == targets.size(0):
            if len(inputs.shape) == 2 and inputs.size(1) > 1:  # 如果是分类问题 [batch_size, num_classes]
                # 使用交叉熵损失，适合分类问题
        ...
        elif inputs.size() == targets.size():
            # 如果维度已经匹配，根据输入类型选择合适的损失函数
            # 二分类或多标签分类问题 - 使用BCE损失
            # 确保输入和目标具有相同的形状
        ...
        # 检查loss是否已定义，如果未定义则使用默认处理
        if loss is None:
            # 尝试简化处理 - 将输入和目标展平后使用MSE
            print(f"警告: FocalLoss未能匹配输入维度 inputs:{inputs.shape}, targets:{targets.shape}, 使用默认MSE损失")
            inputs_flat = inputs.reshape(-1)
            targets_flat = targets.reshape(-1)
            loss = nn.MSELoss(reduction='none')(inputs_flat, targets_flat)
...
def plot_metrics(history, save_dir):
    ...
    # 绘制位置准确率曲线
    plt.subplot(3, 2, 2)
    plt.plot(epochs, history['train_position_acc'], 'b-', label='训练准确率')
    plt.plot(epochs, history['val_position_acc'], 'r-', label='验证准确率')
    plt.title('位置分类准确率')
    plt.xlabel('轮次')
    plt.ylabel('准确率')
    plt.legend()
    ...
    # 绘制F1分数曲线
    plt.subplot(3, 2, 3)
    plt.plot(epochs, history['val_f1'], 'g-', label='验证F1')
    plt.title('位置分类F1分数')
    plt.xlabel('轮次')
    plt.ylabel('F1')
    plt.legend()
    ...
    # 绘制精确率和召回率曲线
    plt.subplot(3, 2, 4)
    plt.plot(epochs, history['val_precision'], 'b-', label='精确率')
    plt.plot(epochs, history['val_recall'], 'r-', label='召回率')
    ...
    # 绘制等级MAE曲线
    plt.subplot(3, 2, 5)
    plt.plot(epochs, history['train_grade_mae'], 'b-', label='训练MAE')
    plt.plot(epochs, history['val_grade_mae'], 'r-', label='验证MAE')
